...
function replayFills(fills) {
  // code -> { qty, cost(수수료 포함 원가), amt(수수료 제외 매수금액), wAt(매수금액×시각 합) }
  const book = {};
  const sells = [];
  for (const f of fills) {
    ...
    const avgCost = b.cost / b.qty;                 // 주당 평단(수수료 포함)
    ...
    sells.push({
      ...
      pnl: Math.round(proceeds - avgCost * qty),
      heldDays: Math.max(0, (f.at - avgBuyAt) / DAY)
    });
    // 이동평균이라 남은 수량 비율만큼 원가·가중시각을 함께 덜어낸다
    // (wAt 와 amt 가 같은 비율로 줄어 평균 매수시각은 그대로 유지된다)
    const ratio = (b.qty - qty) / b.qty;
    b.cost *= ratio; b.amt *= ratio; b.wAt *= ratio; b.qty -= qty;
