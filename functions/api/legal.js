...
export async function onRequestPost(context) {
  ...
  if (!question || question.length > 1000) {
    return json({ error: '질문이 비어있거나 너무 깁니다' }, 400);
  }
  ...
  try {
    ...
    const [aiLawResult, precResult] = await Promise.all([
      searchAILaw(LAW_API_KEY, question),
      searchPrecedents(LAW_API_KEY, keywords),
    ]);

    // Step 2: 상위 법령 조문 상세 조회
    const topLaws = parseAILawResult(aiLawResult).slice(0, 3);
    const lawDetails = await Promise.all(
      topLaws.map(l => getLawArticle(LAW_API_KEY, l.mst, l.joNo))
    );

    // Step 3: 컨텍스트 구성
    const lawContext = buildContext(topLaws, lawDetails, precResult);
    ...
    if (aiData.error) {
      return json({ error: aiData.error.message || 'AI 오류' }, 500);
    }
...
async function searchAILaw(apiKey, query) {
...
async function searchPrecedents(apiKey, query) {
  const params = new URLSearchParams({
    OC: apiKey || '',
    target: 'prec',
    type: 'XML',
...
async function getLawArticle(apiKey, mst, joNo) {
  if (!mst) return '';
  const params = new URLSearchParams({
    OC: apiKey || '',
    target: 'law',
...
function parseAILawResult(xml) {
  ...
  for (const item of items) {
    ...
    if (lawName || mst) results.push({ lawName, mst, joNo, joTitle });
  }
  return results;
}
